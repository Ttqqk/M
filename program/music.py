...
async def play(c: Client, m: Message):
    ...
    if do.count("left") or do.count("Bad Request: user not found"):
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    lnlineKeyboardButton(text="قناة البوت", callback_data="https://t.me/{UPDATES_CHANNEL}"), 
                ]
            ]
        )
        if m.sender_chat:
        ...
        if not a.can_invite_users:
            await m.reply_text("اعطيني صلاحياتِ:" + "\n\n» ❌__إضافة مستخدمين__")
            return
        try:
        ...
        if replied:
                        ...
                        if veez == 0:
                            ...
                            if chat_id in QUEUE:
                                ...
                                try:
                                    ...
                                    await suhu.delete()
                                    await m.reply_text(f"🚫 فشل فتح الاتصال الرجاء فتح الاتصال يدوي : `{ep}`")
            ...
            if len(m.command) < 2:
                ...
                search = ytsearch(query)
                if search == 0:
                    await suhu.edit("❌ **فشل العثور على نتائج**")
                else:
                    songname = search[0]
                    url = search[1]
                    duration = search[2]
                    ...
                    if veez == 0:
                        ...
                        if chat_id in QUEUE:
                            ...
                            await m.reply_photo(
                                photo=thumbnail,
                                caption=f"💡 ***-› تمت إضافية الى قائمة** `{pos}`\n\n🏷 **-› اެݪاެسم:** [{songname}]({url})| موسيقى`\n** الوقتّ:** `{duration}`\n🎧 **-ٛ» طلب الحلو** {requester}",
                                reply_markup=keyboard,
                            )
                        else:
                            try:
                                await suhu.edit("🔄 **جاري الانظمام الإتصال**")
                                await call_py.join_group_call(
                                    chat_id,
                                    AudioPiped(
                                        ytlink,
